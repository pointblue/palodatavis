...
# calculate average net hours per month
effort <- read_csv(nethrspath, 
...
# banding data
captures <- read_csv(bandpath, col_types = cols_only(loc = 'f',
                                         date = col_date(format = '%m/%d/%Y'),
                                         size = 'f',
                                         code = 'f',
                                         bandnumb = 'n',
...
dat <- left_join(captures, effort, by = c('month', 'year')) %>% 
  ...
  select(spec, monthlab, month, rate) %>% 
  # calculate loess by species
  nest(data = c(monthlab, month, rate)) %>% 
  mutate(mod = purrr::map(data, loess, formula = rate ~ month, span = 0.55),
         smooth = purrr::map(mod, `[[`, 'fitted')) %>% 
  select(-mod) %>% 
  unnest(cols = c(data, smooth)) %>% 
  group_by(spec, monthlab, month, smooth) %>% 
  summarize(maxrate = max(rate),
            meanrate = mean(rate)) %>% 
  ungroup() %>% 
  mutate(smooth = case_when(maxrate == 0 ~ 0,
                            smooth < 0 ~ 0,
                            TRUE ~ smooth),
...
<div class="column">
<figure>
![](../images/gcsp_gls_annotated.jpg "Golden-crowned Sparrow carrying a light-level geolocator")
<figcaption>
...
<figure>
<iframe src="migration_map.html" style="border: none;" width = 95%; height = 450px;></iframe>
<figcaption>
*Each point represents an individual bird we tracked on their migration. The coordinates are the average of several points collected throughout the season they were away from Palomarin.*
</figcaption>
</figure>
...
<div class="column">
<figure>
![](../images/MGWA-with-band_by-Stefanie-Krantz_compressed.jpg "A banded MacGillivray’s Warbler")
<figcaption>
...
<figure>
![](../images/KTCH2018_summer_compressed.jpg "Palomarin interns and staff, spring/summer 2018")
<figcaption>
